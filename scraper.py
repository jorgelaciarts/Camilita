...
def fetch_jobs() -> dict:
    ...
    try:
        ...
        if len(general_jobs) == 0:
            ...
            idx = html.lower().find("abogado")
            print(f"[debug] contiene 'abogado' (texto plano)?: {idx != -1}")
            if idx != -1:
                print(f"[debug] contexto alrededor de 'abogado': ...{html[max(0, idx-200):idx+200]}...")
            scripts = soup.find_all("script")
            print(f"[debug] total de <script> tags: {len(scripts)}")
            for i, s in enumerate(scripts):
                s_id = s.get("id", "")
                s_type = s.get("type", "")
                content_len = len(s.string) if s.string else 0
                if content_len > 500:
                    print(f"[debug] script #{i}: id='{s_id}' type='{s_type}' largo={content_len}")
